...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderPatientDashboard(userObj) {
    ...
    // Populate Next Pending Medicine Reminder
    const dashNextMed = document.getElementById("dashNextMed");
    if (dashNextMed && window.DemoEngine) {
      const storedReminders = window.DemoEngine.getStoredReminders();
      const pendingRem = storedReminders.find(r => r.status === "Pending");
      if (pendingRem) {
        dashNextMed.textContent = `${pendingRem.medicine} (${pendingRem.time})`;
      } else {
    ...
    if (dashNearbyHospitalsList) {
      const defaultHospitals = window.DemoEngine ? window.DemoEngine.hospitals : [];
      ...
      renderDashHospitals(defaultHospitals);
  ...
  fetch("http://localhost:5000/api/health")
    ...
    .then(data => {
      if (systemStatusBadge && statusModeText && data.status === "healthy") {
        ...
        statusModeText.textContent = "Live API Connected";
      }
    })
    .catch(() => {});
});
